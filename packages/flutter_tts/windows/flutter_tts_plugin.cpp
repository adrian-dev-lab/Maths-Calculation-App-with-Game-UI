...
HWND g_platform_hwnd = NULL;
#define WM_TTS_COMPLETE (WM_USER + 1)

// Forward declare WndProc - defined per-section
LRESULT CALLBACK TtsWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
	if (msg == WM_TTS_COMPLETE) {
		if (methodChannel) methodChannel->InvokeMethod("speak.onComplete", nullptr);
		return 0;
	}
...
HWND CreateMessageWindow() {
	WNDCLASSEX wc = {};
	wc.cbSize = sizeof(wc);
	wc.lpfnWndProc = TtsWndProc;
	wc.lpszClassName = L"FlutterTtsMsgWnd";
	wc.hInstance = GetModuleHandle(nullptr);
	RegisterClassEx(&wc);
	return CreateWindowEx(0, L"FlutterTtsMsgWnd", L"", 0, 0, 0, 0, 0, HWND_MESSAGE, nullptr, wc.hInstance, nullptr);
}
...
namespace {
	...
	void FlutterTtsPlugin::RegisterWithRegistrar(
		...
		if (!g_platform_hwnd) g_platform_hwnd = CreateMessageWindow();
	}
	...
	void FlutterTtsPlugin::addMplayer() {
		...
		auto mEndedToken =
			mPlayer.MediaEnded([=](Windows::Media::Playback::MediaPlayer const& sender,
				...
				{
				    if (g_platform_hwnd) PostMessage(g_platform_hwnd, WM_TTS_COMPLETE, 0, 0);
...
namespace {
	...
	void FlutterTtsPlugin::RegisterWithRegistrar(
		...
		registrar->AddPlugin(std::move(plugin));
		g_platform_hwnd = CreateMessageWindow();
	}
